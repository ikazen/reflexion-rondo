...
- **현재 구현(BON-191):** `runtime/isolate.py`가 `runtime/runner.py`를 bubblewrap(`bwrap`) subprocess로 실행한다. `--unshare-net`으로 네트워크 차단, `--ro-bind`/`--tmpfs`로 FS 화이트리스트, rlimit(AS/CPU) + 600s timeout 병행. bwrap 부재 시(로컬 mac) env allowlist + rlimit + timeout 폴백.
...
- 근거:
  ...
  - **격리 = subprocess bwrap `--unshare-net`** (ADR-013의 "컨테이너 vs nsjail" TBD 확정, BON-191): 초기 설계의 "Docker `--network none`"은 컨테이너 레벨을 의미했으나, eval/task 컨테이너 자체는 Postgres/MinIO/Ollama 접근에 네트워크가 필요하다. 격리 경계는 **생성 코드 subprocess**. bubblewrap `--unshare-net`으로 네트워크 차단, `--ro-bind`/`--tmpfs`로 FS 화이트리스트를 subprocess 레벨에서 실현. eval 컨테이너는 시크릿 마운트 없음(secrets는 Airflow Variable env로만 주입, allowlist가 제거) — 참고로 이미지에 SOPS 암호문이 포함되나 키 없이 평문 복호화 불가. mem/cpu/timeout 상한으로 OOM 리스크를 흡수. OOM·타임아웃은 워커 사망이 아니라 `error_trace`→교훈이 된다.
...
| 격리 런타임 | bwrap `--unshare-net` + FS ro-bind + mem/cpu/timeout | 구현 완료, ADR-017 (BON-191) |
