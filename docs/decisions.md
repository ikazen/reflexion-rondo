...
- 갱신(2026-09-05, #287): 이 ADR이 지운 것은 "옛 스케일 vs 새 스케일 비교 불가"라는
  증상이지, "새 스케일 자체가 오염됐는지"는 검증하지 않는다 — remeasure는 그 시점의
  `load_train()` 결과가 뭐가 됐든 그걸로 재측정하고 지문을 그 결과에 맞춰 갱신할 뿐이다.
  실제로 2026-08-26 이 ADR의 remeasure가 s4e11에 돌았을 때 `EXTRA_TRAIN_PATHS`의 twin
  중복(#228 원인, #287에서 확정)은 그대로 남아 있었고, 그 결과 remeasure 이후
  새로 확정된 pipeline 3건도 여전히 오염 데이터 기준(cv 0.94392~0.94983, 세계 1위 LB
  0.94488 근접·초과)으로 승격됐다 — "지문이 일치하니 정상"이라는 판정과 "그 지문이 가리키는
...
양상이 달라 이번 범위 밖.
- 갱신 (2026-09-04, #283): 1차 적용(s5e4·s4e11 = 10800s)을 철회한다. v1.6.12 컷오버
전후 23h 실측 — s5e4 킬 비율 40%→38%로 안 내리고 킬 1건당 소각량만 3배(킬 총량
17.0→37.0 CPU-h/23h), s5e4 전체 CPU 소비 2배. 상향으로 새로 통과한 성공 attempt 8건은
전부 neutral/regression, 확정 pipeline 0. s5e4 median CPU가 예산을 그대로 추종했다
(902s→3599s, 예산 900→3600→10800). "예산에 맞춰 작업이 커질 뿐 산출은 안 는다"로
결론. s4e11의 10800s는 POST 23h 동안 한 번도 안 물려(킬 0건, max CPU 2226s) 무의미했다.
두 상수를 config에서 제거하고 전역 `DEFAULT_CPU_BUDGET_SECS`(3600) 안전망으로 복귀한다.
`getattr(comp, "CPU_BUDGET_SECS", None)` 배선 자체는 유지 — s6e8처럼 실제 산출을 내면서
900s 벽에 막히는 대회(#176)를 위한 경로다. s5e4는 #283/ADR-045로 동결.

---

## ADR-045 — deep tier를 3개에서 2개로 좁힌다 (s5e4 동결, #283)

- 결정: ADR-043의 deep tier 3개(s6e8·s5e4·s4e11) 중 s5e4를 `ACTIVE=False`로 동결한다.
유지: s6e8(binary/auc)·s4e11(binary/accuracy). attempts 이력은 보존한다(ADR-025·032·043과 동일).
- 근거: 2026-09-04 상태 점검 실측. s5e4는 ADR-043이 동결한 두 대회보다 지표가 나쁘다.

  | 대회 | 7일 attempt | 7일 CPU-h | 에러율 | jump | 30일 확정 pipeline |
  |---|---|---|---|---|---|
  | s5e4 | 267 | 189.9 (fleet 32%) | 51% | 1 | 0 |
  | s6e8 | 258 | 137.7 | 26% | 6 | 0 |
  | s4e11 | 312 | 87.5 | 22% | 137 | 3 |
  | (동결됨) s4e12 | 245 | 120.9 | 16% | 0 | 1 |
  | (동결됨) s4e10 | 247 | 61.5 | 10% | 0 | 0 |

  s5e4는 fleet CPU의 32%를 태우면서 에러율 51%, 7일 확정 pipeline 0건이다. ADR-044의
  CPU 예산 상향(3600→10800)도 킬 비율을 못 내렸고 소각량만 3배로 키웠다(위 ADR-044 갱신).
  마지막 성공 제출은 08-26(LB 12.82498)이고 그 전 08-04(12.5966)보다 나빠졌다.
- 잃는 것: deep tier에서 regression task_type이 0개가 된다(s5e4가 마지막 회귀 대회).
  ADR-043이 이미 rmsle(s4e12)를 뺐고 이제 rmse(s5e4)까지 빠져 회귀 트랙 전체가 없어진다.
  새 역량이 회귀에서 통하는지는 판별 불가 — #39 백로그에서 회귀 대회 1개 교체 온보딩이 후속.
- 동결로 잃는 제출: 없다. s5e4 유효 pipeline 8건 중 제출 가능한 것은 08-01 `promote` 1건
  (이미 3회 제출)뿐이고 나머지는 전부 `unverifiable:eval_error`라 #254/#270 하드 제외로
  이미 auto-submit 대상이 아니다.
- ADR-032·043은 삭제하지 않는다(living document) — 이 ADR이 그 갱신 사실이다. deep tier가
  2개로도 정체되면 대회 교체 또는 fleet 재확장을 재검토한다.
...
## ADR-047 — 대회 신호 대 잡음비(SNR)를 온보딩·동결 판단의 보조 축으로 쓴다 (#291)

- 결정: `competition_snr` 뷰(`store/schema.sql`) — 리더보드 p50→p90 원점수 격차를
  실측 fold 표준오차(SEM 공식, `sqrt(avg(cv_fold_var)/n_splits)`)로 나눈 값. 실측:

  | 대회 | p90-p50 격차 | SNR |
  |---|---|---|
  | s4e11 | 0.00251 (accuracy) | 4.17 |
  | s4e10 | 0.01925 (accuracy) | 28.66 |
  | s4e12 | 0.04074 | 36.29 |
  | s5e4 | 1.17503 | 100.79 |
  | s6e8 | 0.00553 (auc) | 21.93 |

- 근거: s4e11은 SNR 4.17 — p50과 p90의 실제 경쟁 격차가 fold 노이즈의 4배 수준밖에
  안 돼, 후보 간 cv 우열을 신뢰성 있게 못 가른다(fold 노이즈 몇 배 안에서 순위가
  흔들릴 수 있음). s4e10/s4e12/s5e4(모두 ADR-043/045로 동결됨)는 SNR이 훨씬 높다 —
  동결 사유(에러율·CPU 소각·확정 pipeline 0)와 겹치지 않는 별도 신호라는 뜻이다.
  s4e11은 확정 pipeline·jump가 fleet에서 가장 활발한 대회인데도 SNR이 가장 낮다 —
  "잘 돌고 있다"가 "순위를 신뢰할 수 있다"와 다른 질문임을 보여준다.
- n_splits는 `config.N_SPLITS`(Python 설정, 기본 5)를 SQL 뷰에서 직접 못 읽어
  `raw.attempts.fold_scores` 배열 길이의 평균으로 대체 실측한다 — s5e3만 10으로
  다르고 나머지는 5라 이 근사가 유효하다. avg_fold_var/avg_n_splits는 전체 이력
  평균(시간 윈도우 없음)이라 최근 pipeline 변경으로 fold 분산이 달라졌으면 다소
  지연 반영된다.
- 적용: 온보딩(#39 백로그)·동결 판단에 ADR-043/045의 기존 축(에러율, CPU 소각량,
  확정 pipeline 산출)에 더해 SNR을 추가 축으로 쓴다. 낮은 SNR 자체가 즉시 동결
  사유는 아니다 — s4e11처럼 fleet에서 가장 활발한 대회일 수도 있으므로, "더 투자할
  가치" 판단이 아니라 "cv 기반 순위·게이트를 얼마나 신뢰할지"에 대한 보정 신호로
  쓴다.
- s4e11의 거취는 이 ADR에서 결정하지 않는다 — #287 twin 오염 제거 직후라 아직
...
## ADR-049 — deep tier를 2개에서 1개로 좁힌다 (s4e11 동결, #316)

- 결정: ADR-043/045의 deep tier 2개(s6e8·s4e11) 중 s4e11을 `ACTIVE=False`로
동결한다. 유지: s6e8(binary/auc). attempts 이력은 보존한다(ADR-025·032·043·045와
동일). 동결과 동시에 회귀(regression) 대회 1개를 #39 백로그에서 교체 온보딩한다
(#317, 별도 이슈) — deep tier를 1개로 줄이는 게 아니라 s4e11 → 신규 회귀 대회로
교체한다.
- 근거: 2026-09-15 상태 점검 실측. ADR-043/045가 s4e10/s4e12/s5e4를 동결할 때 쓴
바로 그 기준(30일 확정 pipeline 0건 + 불균형한 CPU 소비)에 s4e11이 걸린다.

  | 지표 | s4e11 | s6e8(대조) |
  |---|---|---|
  | 확정 유효 pipeline 최근 33일 | 0건 (마지막 08-13) | 다수 (08-17/19/22×2/09-08/09-11) |
  | lifetime attempt / CPU-h | 2,141 / 213 | 2,577 / 967 |
  | 최고 LB → 최근 제출 | 0.94227(08-29) → 0.93821(09-04, 하락) | 상승 추세(0.96625→0.96781) |
  | 제출 중단 | 09-04 이후 0건(확정 pipeline 없음) | 계속 진행 중 |
  | SNR(ADR-047) | 4.02 (fleet 최저) | 15.31 |
  | gap_to_p90(ADR-047) | 0.00144 (fleet 최소) | 0.00336 |
  | v1.6.16(ADR-048) 배포 후 jump(09-13~14) | 0건 | 0건(관측 기간 짧음) |

  s4e11은 fleet에서 가장 많은 컴퓨트를 받으면서(attempt 수 기준 최다) 33일째
  산출이 없고, LB도 개선이 아니라 악화 방향이다. gap_to_p90이 fleet 최소라는 건
  이론적으로도 더 딸 여지가 거의 없다는 뜻 — SNR 4.02(fold 노이즈 대비 신호가
  fleet에서 가장 약함)와 함께 보면 "아직 안 풀렸다"가 아니라 "이 대회 데이터로는
  현재 접근법의 한계에 가깝다"에 더 부합한다.
- ADR-048(fit 계약 통일)이 s4e11을 구제할 가능성은 남아 있었으나, 배포 후 이틀
  관측 기간이 짧아 이 ADR의 결정에 반영하지 않는다 — 결과가 좋았다면 향후 ADR로
  재개(un-freeze) 가능하다(ADR-032 이후 매번 그래왔듯).
- 잃는 것: deep tier에 binary/accuracy 조합이 없어진다(s6e8은 binary/auc). 새
  역량이 accuracy 지표에서 통하는지는 당분간 판별 불가 — #309(accuracy 확률 경로
  복원)도 이 공백과 맞물려 있어 이번 Milestone 범위 밖으로 미룬다.
- 동결로 잃는 제출: 없다. s4e11의 유효 확정 pipeline이 0건이라 애초에 제출
  대상이 없다.
...
- 결정: ADR-049로 편입한 s5e2를 `ACTIVE=False`로 동결하고, ADR-045로 동결했던
  s5e4를 `ACTIVE=True`로 재활성한다. 유지: s6e8(binary/auc). deep tier 구성원
  숫자는 1개 그대로(s6e8 + 회귀 1개) — s5e2 → s5e4 교체다.
- 근거: 2026-09-18 상태 점검 실측. ADR-049는 s5e2를 "SNR 8.17로 s5e10(1.12)보다
  우위"라는 이유로 선택했는데, 재활성 3일 만에 그 선택보다 명백히 나은 후보가
  fleet 안에 이미 있었다는 게 드러났다 — 애초 비교 대상이 동결군 전체가 아니라
  s5e10 하나뿐이었던 탓.

  | 대회 | 상태 | attempt | jump | 확정 pipeline | SNR | gap_to_p90(원점수) |
  |---|---|---|---|---|---|---|
  | s5e2 | ACTIVE(09-15~) | 960(3일) | 0 | 1(07-31, 튜닝 불가) | 8.2 | 0.258 |
  | s5e4 | 동결(09-03~) | 1,305 | 18 | 8 | **100.8** | **0.730** |
  | s4e12 | 동결 | 1,226 | 19 | 5 | 36.3 | 0.015 |
  | s4e10 | 동결 | 3,426 | 63 | 16 | 28.7 | 0.010 |

  s5e2는 재활성 이후 3일간 960 attempt에 jump 0건이고, 유일한 확정 pipeline
  (07-26, model_spec 도입 이전 자유형)이 `infer_registry_model`(ADR-035/#252)의
  정적 추론 조건을 통과 못 해 Optuna 튜닝 레인(ADR-050)이 매시간 즉시 `ValueError`로
  실패한다(#327, 09-17~18 10회 연속 실측). 이건 "아직 성과가 없다"가 아니라
  **현재 fleet 자동화 도구체계(ADR-050 튜닝 레인)의 혜택을 원천적으로 못 받는
  구조**라는 점에서 ADR-043/045/049가 다뤄온 "산출 정체" 사유와는 다른 축이다.
  s5e4는 SNR 100.8로 fleet 전체(동결 포함) 2위 s4e12의 3배이고, gap_to_p90도
  fleet 최대 — CV 신호가 가장 신뢰할 수 있고 개선 여지도 가장 크다. confirmed
  pipeline도 `LGBMRegressor(random_state=ctx.seed, **params)` 단일 레지스트리
...
- 잃는 것: 없음 — s5e2는 lifetime 확정 pipeline이 07-26 1건뿐이고 그마저 튜닝
  ...
  043·045·049와 동일).
- 동결로 잃는 제출: 없다. s5e2의 유일한 확정 pipeline은 이미 제출 이력이 있고
  (09-15), 신규 확정 없이는 추가 제출 대상 자체가 없었다.
- #327(s5e2 튜닝 DAG 매시간 실패)은 이 동결로 해소된다 — `_sweep_idle_tuning`/
  `_maybe_trigger_tune`(`bin/run_daemon.py`) 둘 다 `active_competition_ids()`
  필터를 타므로 별도 코드 수정 없이 트리거 자체가 멈춘다.
- ADR-032·043·045·049는 삭제하지 않는다(living document) — 이 ADR이 그 갱신
  사실이다.

---

## ADR-052 — s5e4 재활성 실패는 예산이 아니라 attempt 단가 문제다 (#340)

- 결정: `config/competitions/s5e4.py`의 `MAX_TRAIN_ROWS`를 500,000 → 150,000으로,
  `N_SPLITS`를 기본값 5 → 3으로 낮춘다. 예산(`CPU_BUDGET_SECS`)은 건드리지 않고
  전역 기본값 3600s를 유지한다.
- 근거: ADR-051이 "재활성 시 예산 재조정 불필요"라고 예측한 것과 반대로,
  재활성 1일 실측(09-19)에서 attempt 70%가 CPU 예산 kill로 죽었고 fleet CPU
  89h 중 57.5h(65%)를 이 대회 하나가 소각했다 — ADR-045가 동결했던 실패모드
  (당시 킬 비율 40%대)가 재현을 넘어 악화됐다.
- ADR-044(CPU 예산 3600→10800 상향)는 이미 s5e4에서 실패로 결론났다(킬
  비율은 그대로, 소각량만 3배로 커짐 — ADR-045 참고). 즉 "예산을 늘린다"는
  축은 이미 시도되고 기각된 상태다. 남은 개입 축은 attempt 단가(행 수 ×
  fold 수)뿐이다.
- 500k행 × 5-fold는 다른 어떤 활성 대회보다 무거운 조합이다(s6e8/s4e11은
  기본 5-fold지만 행 수가 이 정도로 크지 않음). 행 수를 3.3배(500k→150k),
  fold 수를 1.67배(5→3) 줄이면 이론상 attempt당 계산량이 약 5.5배 줄어든다.
- 대안으로 검토하고 기각: (1) 다시 동결 — ADR-051이 이미 s5e2 대비 s5e4의
  SNR/gap_to_p90 우위를 확인했고, 회귀 트랙 슬롯이 비면 포트폴리오가 1개
  구성원으로 더 좁아진다. (2) 예산 재상향 — ADR-044가 이미 기각. (3) 대회
  자체 재동결 없이 방치 — CPU 낭비가 fleet 전체 산출을 계속 깎아먹는다.
- 필수 후속: `MAX_TRAIN_ROWS`/`N_SPLITS` 변경은 train90 구성을 바꿔
  `train_fingerprint` 가드(ADR-040/#258)를 발동시킨다. 배포 후
  `bin/establish_baseline.py --competition playground-series-s5e4 --remeasure`로
  재측정해야 attempt가 재개된다.
- 롤백 조건: 배포 후 24h 관측에서 CPU kill 비율이 s6e8 수준(약 12%)까지 안
  내려오면, 단가 축소로도 안 풀리는 문제로 보고 ADR-045 방식으로 재동결한다.
