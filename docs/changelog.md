...
- v1.6.11 배포 후 24h 실측: #278 가드가 의도대로 동작 — s4e11 팬텀 baseline(0.9681) 소멸로 jump
  오탐 붕괴(126건/7일 → 8건), neutral 정상 복귀(0 → 19건), 확정 pipeline 2건, 제출 2건(08-29 이후
  첫 제출). CV↔LB 역상관(-0.813)은 #267 이전 스케일 혼재 아티팩트로 확인 — post-#267 2건은 +1.000,
  #258 재오픈 근거 아님.
- #269: 8일 fleet 679 CPU-h 중 CPU 예산 킬이 215 CPU-h(32%), 확정 pipeline 산출 2건. s5e4 킬
  집단(120건)의 CPU/wall 비율 median 2.86 ≈ 성공 집단(156건) 2.95 — 병렬성 폭주가 아니라 같은
  병렬도의 무거운 정상 작업이다(폭주 가드는 #162 정적 거부가 담당). `config/competitions/s5e4.py`·
  `s4e11.py`에 `CPU_BUDGET_SECS = 10_800`(관측용 임시값). 상수는 이미 전 진입점에 배선돼 있어 새
  코드 없음. ADR-044. 전역 `DEFAULT_CPU_BUDGET_SECS`(3600)는 안전망으로 유지.
- #274: s4e10·s4e12를 `ACTIVE=False`로 동결(deep tier 5→3, 유지 s6e8·s5e4·s4e11). 두 대회 30일
  확정 pipeline 0/1건, 30h max gain 순수 노이즈(+0.0004/-0.0001), 41 CPU-h/30h가 순 손실. confirm
  게이트는 정상(노이즈 정확히 거부) — 개선이 안 나오는 게 문제. 동결로 잃는 제출 없음(미제출 유효
  pipeline 전부 unverifiable:*, 이미 #254/#270 하드 제외). ADR-043 — ADR-032의 rmsle 다양성 상실은
  트레이드오프로 수용.
- 관측(48h): s5e4/s4e11 CPU 킬 비율 하락 + 킬 집단 CPU/wall 비율 2.9 유지(폭주 안 풀림) → ADR-044
  영구값 확정. s4e10/s4e12가 큐 리필에서 빠지는지 확인.
...
- #254: v1.6.0 배포 직후 첫 auto-submit 배치(10건) 중 4건이 Kaggle에 닿기도 전에
  ...
  만드는 `load_base_snapshot`이 2026-08-01 이전 승격분에는 `materialized_code` 스냅샷(#89)이
  없어 replay 폴백을 타고, 그 replay는 materialize 합성 규칙이 바뀐 뒤(ADR-037/#232) 승격 당시와
  다른 코드를 만들어 sha 검증에 걸린다. fail-hard 가드 자체는 옳다 — 평가와 다른 base로 제출하면
  조용히 열화된 예측이 나간다.
- `bin/api.py:_unsubmitted_confirmed`의 정렬 1차 키를 cv에서 "직전 승격분 스냅샷 보유 여부"로
  바꿨다. 제외가 아니라 후순위다 — 스냅샷 없이도 replay가 재현되는 경우가 실제로 있다(첫 배치의
  s4e12 1건). 실측 상관: 직전 스냅샷 있는 5건 전건 성공, 없는 5건 중 4건 실패.
- 이 정렬이 없으면 s4e10(백로그 12건 전부 스냅샷 없음)과 s5e4가 매일 일일 예산을 전량 헛되이
  태우고 LB 갱신이 영영 안 된다 — #233 완료 기준("deep tier 전 대회 주 1회 이상 LB 갱신")이
  깨진다.
- 2026-08-01 이전 이력 자체를 제출 가능하게 만드는 문제는 #254에서 계속 다룬다.
